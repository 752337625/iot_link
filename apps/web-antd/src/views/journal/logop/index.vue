...
import LogOpModel from './logOpModel.vue';
...
const exportHandle = async () => {
  ...
  const res = await sysLogOpExport({
    ...value,
...
const [Modal, modalApi] = useVbenModal({
  connectedComponent: LogOpModel,
});
/**
 *打开详情
 */
const openHandle = async (row: Record<string, string>) => {
  const res = await sysLogOpDetail({ id: row.id });
  modalApi.setData({
    message: res.message,
